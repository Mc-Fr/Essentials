...
public class McFrDate {
  ...
  private static final String[] MONTHS_NAMES = { "Emeral", "Carminal", "Roséal", "Auréal", "Ambridor", "Oridor", "Mercuridor", "Pyridor", "Cendraire",
      "Cyanaire", "Pourpraire", "Anthraire", "Mauvose", "Sombrose", "Lazurose", "Fumose" };
  ...
  public void actualize() {
    ...
    double daylightProportion = MEAN_DAY_PROPORTION
        + 0.05f * (1 - Math.cos(2f * Math.PI * (1f * this.month + 1f * (this.day - 1) / DAYS_IN_MONTH) / MONTHS_IN_YEAR));
  ...
  public boolean isWeatherDelayPassed(McFrDate current) {
    long hours = ((this.year * MONTHS_IN_YEAR + this.month) * DAYS_IN_MONTH + this.day) * HOURS_IN_HALF_DAY * 2 + getHour();
  ...
  public String toString() {
    return this.day + " " + MONTHS_NAMES[this.month - 1] + " " + this.year + " - " + getHour() + "h";
  }
}

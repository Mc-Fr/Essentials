...
public class CareCenterCommand extends AbstractCommand {
  ...
  public CommandSpec getCommandSpec() {
    ...
    return CommandSpec.builder()
            ...
            .executor(this)
            .children(getChildrenList(new AddCenter(),
                new RemoveCenter(),
                new ListCenters(),
                new GiveFaction(),
                new RemoveFaction(),
      ...
      if (src instanceof Player) {
        if (args.hasAny("nom") && args.hasAny("rayon") && args.<Integer>getOne("rayon").get() > 0) {
          if (careService.addCenter(args.<String>getOne("nom").get(), ((Player) src).getLocation(), args.<Integer>getOne("rayon").get(),
            ...
            if (Sponge.getServiceManager().provide(CareService.class).isPresent()) {
              src.sendMessage(Text.of(TextColors.YELLOW,
                  "Une zone porte déjà le nom " + args.<String>getOne("nom").get() + " ou la faction spécifiée n'existe pas."));
            } else {
              src.sendMessage(Text.of(TextColors.RED, "Le système de centres de soins n'a pas été correctement chargé."));
            }
          }
        } else {
          src.sendMessage(Text.of(TextColors.RED,
        ...
        if (optCareService.isPresent()) {
          ...
          if (optCenter.isPresent()) {
            if (args.<String>getOne("confirmation").isPresent() && args.<String>getOne("confirmation").get().equals("confirm")) {
              ...
              src.sendMessage(Text.of(TextColors.YELLOW, "Le centre de soins a été effacé : " + args.<String>getOne("nom").get()));
            } else {
              src.sendMessage(Text.of(TextColors.YELLOW, "Le centre de soins a été trouvé : " + args.<String>getOne("nom").get()
                  + ". Merci de recommencer la commande en ajoutant \"confirm\" à la fin pour confirmer la suppression."));
            }
          } else {
            src.sendMessage(Text.of(TextColors.YELLOW, "Il n'y a aucun centre de soins portant le nom : " + args.<String>getOne("nom").get()));
          }
        } else {
          src.sendMessage(Text.of(TextColors.RED, "Le système de centres de soins n'a pas été correctement chargé."));
        }
      } else {
        src.sendMessage(Text.of(TextColors.RED, "Merci de renseigner les arguments : /carecenter removecenter <nom> confirm"));
      }
  ...
  static class ListCenters extends AbstractCommand {
    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
      ...
      if (optCareService.isPresent()) {
        ...
        paginationService.builder().title(Text.of(TextColors.YELLOW, "Centres de soins")).linesPerPage(10).padding(Text.of(TextColors.GOLD, "="))
  ...
  static class ListFactions extends AbstractCommand {
    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
      ...
      if (optCareService.isPresent()) {
        ...
        PaginationService paginationService = Sponge.getServiceManager().provide(PaginationService.class).get();
        paginationService.builder().title(Text.of(TextColors.YELLOW, "Factions")).linesPerPage(10).padding(Text.of(TextColors.GOLD, "="))
            .contents(texts).sendTo(src);
      } else {
        src.sendMessage(Text.of(TextColors.RED, "Le système de centres de soins n'a pas été correctement chargé."));
      }
